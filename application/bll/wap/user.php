...
class User extends wap{
    ...
    public function getUserInfo($params)
    {
        if(empty($params['uid']) || !is_numeric($params['uid'])){
            return array('code' => 300,'msg' => '无效的用户ID');
        }
        $rs = $this->ci->user_model->dump(array('id' => $params['uid']));
        if(empty($rs)){
            return array('code' => 300,'msg' => '该用户不存在');
        }
        $jf = $this->ci->user_model->getUserScore($params['uid']);
        $money = $this->ci->user_model->get_user_money($params['uid']);
        $rs['jf'] = !empty($jf['jf']) ? $jf['jf'] : 0;
        $rs['money'] = sprintf("%.2f", $money[0]->amount);
        return $rs;
    }
