...
class Foryousave extends wap {
    function __construct() {
        $this->ci = &get_instance();
        $this->ci->load->model('for_you_save_model');
 ...
 */
    public function getlist($params) {
        ...
        $k_sort = $params['sort'];
        $list = $this->ci->for_you_save_model->get_list($params['user_rank']);
        //过滤地区
        $card = array();
        ...
        if (count($card) > 0) {
            ...
            foreach ($card as $k => $v) {
                $card_info = $this->ci->for_you_save_model->get_card_info($v['tag']);
                $card[$k]['picture'] = $picPrefix . $card[$k]['picture'];
    ...
    public function exchange($params) {
        ...
        $point = $this->ci->for_you_save_model->get_info($params['id']);
        ...
        $mobilecard_info = $this->ci->for_you_save_model->get_card_info($point['tag']);
    ...
    private function rand_card_number($p_card_number = '') {
        ...
        if ($this->ci->for_you_save_model->checkCardNum($p_card_number . $tname)) {
