...
class Request {
     ...
     * @author 
     **/
    public function realtime_call($url,$odata,$method = 'POST',$timeout = 6,$aes_key=null,$has_key=null)
    {
        $data = $this->preprocess_data($odata);

        $this->ci->load->library('aes',null,'encrypt_aes');
        $params = array(
            'data' => $this->ci->encrypt_aes->AesEncrypt($data,$aes_key),
            'signature' => $this->ci->encrypt_aes->data_hash($data,$has_key),
        );
        ...
        // 解密
        $data = $this->ci->encrypt_aes->AesDecrypt($response['data'],$aes_key);
    ...
    public function realtime_call_oms($url,$odata,$method = 'POST',$timeout = 6,$aes_key=null,$has_key=null)
    {
        $data = $this->preprocess_data($odata);
